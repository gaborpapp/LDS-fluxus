...
(define (osc_recieve)
    ...
    (let ([cb4 (osc-drain "/mrmr/pushbutton/16/Gas")])
        (when cb4
            (set! b4 cb4)))
    (let ([cb0 (osc-drain "/mrmr/pushbutton/0/Gas")])
        (when cb0
            (set! b0 cb0)))
    (let ([cb6 (osc-drain "/mrmr/pushbutton/3/Gas")])
        (when cb6
            (set! b6 cb6)))
    (let ([cb7 (osc-drain "/mrmr/pushbutton/4/Gas")])
        (when cb7
            (set! b7 cb7)))
    (let ([cb8 (osc-drain "/mrmr/pushbutton/5/Gas")])
        (when cb8
            (set! b8 cb8)))
...
(define tunnel-texture-0 "colors.png")
(define tunnel-texture-1 "D111.png")
(set-num-frequency-bins 1)

(define data-folder "data") ; for textures and title font

(define polyhedron-id 10) ; polyhedra index from the polyhedra list below
(define polyhedron-scale 7)
...
(define (destroyer) (with-pixels-renderer render-buffer (destroy object)))
(define (setter) 
    (begin 
        (let ([num (random 12)])
...
(define (render-tunnel)
    ...
    (with-pixels-renderer render-buffer
        ...
        (with-primitive tunnel
            ;(pdata-index-map! (λ (i c) (ngl (remainder i (+ 1 (inexact->exact (floor (* slide1 10))))))) "c")
            (backfacecull 0)
            (point-width (* 5 (ngl 0)))
            (hint-none)
            ; (hint-wire)
            ...
            (if (> b7 .1)
                (pdata-index-map! (lambda (i p p1 n)
                        (vadd (vmul n (* (gl i) 16 (snoise (vx p1) (vy p1) (vz p1)))) p1))
                    "p" "p1" "n")
        ...
        (with-primitive object
            ...
            (translate (vector (+  (cos(time)) tunnel-outer-radius) 0 (* 0 (sin (time)))))
            ...
            (if (> b1 .5)
                (pdata-map! (lambda (p p1 n)
                        (vmul (vadd (vmul n  (snoise (vx p1) (vy p1) (vz p1))) p) .9))
                    "p" "p1" "n")
                (pdata-map! (lambda (p p1 n)
                        p1)
                    "p" "p1" "n")
        ...
        (with-primitive particles
            ...
            (for ([i (in-range 0 (length tunnel-points) 1)])
                (let ([p (list-ref tunnel-points 0)])
                    (pdata-set! "p" i p))
                )
